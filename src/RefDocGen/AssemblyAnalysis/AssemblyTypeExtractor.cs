...
internal class AssemblyTypeExtractor
{
    ...
    private readonly List<ObjectTypeData> nestedObjectTypes = [];
    private readonly List<DelegateTypeData> nestedDelegates = [];
    private readonly List<EnumTypeData> nestedEnums = [];
    ...
    internal TypeRegistry GetDeclaredTypes()
    {
        ...
        var visibleTypes = assembly
            ...
            .Where(t => AccessModifierHelper.GetAccessModifier(t).IsAtMostAsRestrictiveAs(minVisibility)); // select only the types that are visible
        ...
        return new TypeRegistry(
            objectTypesData.Merge(nestedObjectTypes.ToIdDictionary()), // add the types (including nested ones) to the registry
            enumsData.Merge(nestedEnums.ToIdDictionary()),
            delegatesData.Merge(nestedDelegates.ToIdDictionary()));
    }
    ...
    private ObjectTypeData ConstructObjectType(Type type)
    {
        ...
        this.nestedObjectTypes.AddRange(nestedObjectTypes);
        this.nestedDelegates.AddRange(nestedDelegates);
        this.nestedEnums.AddRange(nestedEnums);
