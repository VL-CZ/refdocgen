...
internal class TypeRegistry : ITypeRegistry
{
    ...
    internal IReadOnlyList<MemberData> GetDeclaredBaseMembers(MemberData member)
    {
        ...
        foreach (var parentType in parentTypes)
        {
            ...
            // the parent type is contained in the type registry
            if (GetDeclaredType(parentId) is TypeDeclaration parentDecl)
            {
                string memberId = member.Id;

                if (member is ICallableMemberData callableMember && callableMember.IsExplicitImplementation)
                {
                ...
                if (parentDecl.AllMembers.TryGetValue(memberId, out var parentMember)) // TODO: explicit interface implementation & inheriting generic type
                {
                    result.Add(parentMember);
                    continue;
                }

                var foundMembers = parentDecl.AllMembers.Where(x => GetInstantiatedMemberId(x.Key, parentType) == member.Id).Select(pair => pair.Value);
